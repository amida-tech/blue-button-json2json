...
var prototype = {
    ...
    dataKeyToInput: function (input, dataKey) {
        ...
        if (typeof dataKey === 'function') {
            return dataKey(input, this.dataKeyFnOptions);
        }
    ...
    run: function (template, input) {
        ...
        if (bbuobject.exists(input) && template.dataKey) {
            input = this.evaluateDataKey(input, template.dataKey);
        }
        if ((input !== null) && template.dataTransform) {
            input = template.dataTransform(input, this.dataKeyFnOptions);
        }
        if (Array.isArray(input)) {
            if (template.single) {
                input = input[0];
